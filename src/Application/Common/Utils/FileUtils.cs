...
{
    private static readonly ILogger Logging = Log.ForContext(typeof(FileUtils));
    ...
    public static bool SafeDelete(this FileInfo fileToDelete)
    {
        if ( fileToDelete.Exists )
            ...
            {
                fileToDelete.Delete();
                Logging.Information("Deleted file {0}", fileToDelete.FullName);
                return true;
            }
            catch ( Exception ex )
            {
                Logging.Error("Unable to delete file: {0}, {1}", fileToDelete.FullName, ex.Message);
            }
